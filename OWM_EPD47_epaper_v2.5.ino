...
//################  VERSION  ##################################################
String version = "4.7in";  // Programme version, see change log at end
//################ VARIABLES ##################################################
...
#define max_readings 8 // Limited to 3-days here, but could go to 5-days = 40
...
float  LongSleepStart = 23.5;  // Don't wakeup until after 07:00 to save battery power
float  LongSleepEnd = 6; // Sleep after 23:00 to save battery power
long StartTime     = 0;
long SleepTimer    = 0; // Sleep timer value
...
void BeginSleep() {
	epd_poweroff_all();
	UpdateLocalTime();
    if (CurrentHour * 60 + CurrentMin >= LongSleepStart*60){
        Serial.println("Start Deep Sleep");
		long LongSleepDuration = 1440 - (CurrentHour * 60 + CurrentMin)  + LongSleepEnd * 60;
		SleepTimer = LongSleepDuration * 60 + Delta; //Some ESP32 have a RTC that is too fast to maintain accurate time, so add an offset
	}
	else if(LongSleepEnd * 60 >= CurrentHour * 60 + CurrentMin){
        Serial.println("Start Deep Sleep 2");
		long LongSleepDuration = (LongSleepEnd * 60) -  (CurrentHour * 60 + CurrentMin);
		SleepTimer = LongSleepDuration * 60 + Delta; //Some ESP32 have a RTC that is too fast to maintain accurate time, so add an offset
	}
    else{
        Serial.println("Start Normal Sleep");
...
void loop() {
	// Nothing to do here shuts down before its reached
}
void setup() {
	InitialiseSystem();
	if (StartWiFi() == WL_CONNECTED && SetupTime() == true) {
		byte Attempts = 1;
		bool RxOnecall = false;
		WiFiClient client;   // wifi client object
		while (RxOnecall == false && Attempts <= 2) { // Try up-to 2 time for Weather and Forecast data
			if (RxOnecall == false) RxOnecall = obtainWeatherData(client, "onecall");
			Attempts++;
		}
		Serial.println("Received all weather data...");
		if (RxOnecall) { // Only if received Weather proceed
...
String GetUnixDay(int unix_time) {
	// Returns either '21:12  ' or ' 09:12pm' depending on Units mode
	time_t tm = unix_time;
	struct tm *now_tm = localtime(&tm);
	char output[5];
...
boolean UpdateLocalTime() {
	...
	else
	{
		...
		strftime(update_time, sizeof(update_time), "%I:%M%p", &timeinfo);        // Creates: '@ 02:05:49pm'
		sprintf(time_output, "%s", update_time);
	}
	Date_str = day_output;
...
bool obtainWeatherData(WiFiClient & client, const String & RequestType) {
	const String units = (Units == "metric" ? "metric" : "imperial");
	client.stop(); // close connection before sending a new request
	HTTPClient http;
	String uri = "/data/2.5/" + RequestType + "?lat=" + Lat + "&lon=" + Lon + "&exclude=alert,minutely,hourly&APPID=" + apikey + "&mode=json&units=" + units + "&lang=" + Language;
...
bool DecodeWeather(WiFiClient& json, String Type) {
	...
	if (Type == "onecall") {
		...
		WxConditions[0].Main0       = root["current"]["weather"][0]["main"].as<char*>();           Serial.println("Main: " + String(WxConditions[0].Main0));
		WxConditions[0].Forecast0   = root["current"]["weather"][0]["description"].as<char*>();    Serial.println("For0: " + String(WxConditions[0].Forecast0));
		WxConditions[0].Icon        = root["current"]["weather"][0]["icon"].as<char*>();           Serial.println("Icon: " + String(WxConditions[0].Icon));
		...
		WxConditions[0].Low         = root["current"]["temp_min"].as<float>();                     Serial.println("TLow: " + String(WxConditions[0].Low));
		WxConditions[0].High        = root["current"]["temp_max"].as<float>();                     Serial.println("THig: " + String(WxConditions[0].High));
        ...
        WxConditions[0].UVIndex     = root["current"]["uvi"].as<int>();                            Serial.println("uvi: " + String(WxConditions[0].UVIndex));
		WxConditions[0].Rainfall	= root["current"]["rain"]["1h"].as<float>();						Serial.println("rain: " + String(WxConditions[0].Rainfall));
		WxConditions[0].Snowfall	= root["current"]["snow"]["1h"].as<float>();						Serial.println("rain: " + String(WxConditions[0].Snowfall));

		JsonArray list = root["daily"];
		for (byte r = 0; r < max_readings; r++) {
			WxForecast[r].Dt                = list[r]["dt"].as<int>();
			WxForecast[r].Sunrise           = list[r]["sunrise"].as<int>();
			WxForecast[r].Sunset            = list[r]["sunset"].as<int>();
			WxForecast[r].MoonRise          = list[r]["moonrise"].as<int>();
			WxForecast[r].MoonSet          = list[r]["moonset"].as<int>();
            ...
            WxForecast[r].UVIndex           = list[r]["uvi"].as<float>();               		Serial.println("uvi: " + String(WxForecast[r].UVIndex));
			WxForecast[r].Rainfall			= list[r]["rain"].as<float>();						Serial.println("rain: " + String(WxForecast[r].Rainfall));
			WxForecast[r].Snowfall			= list[r]["snow"].as<float>();						Serial.println("rain: " + String(WxForecast[r].Snowfall));
		}
		WxConditions[0].Low = WxForecast[0].Low;
		WxConditions[0].High = WxForecast[0].High;
		...
		if (Units == "imperial") Convert_Readings_to_Imperial();
	}
	return true;
}
float GetRoopTemp(){
    ...
    float temperatureF = sensors.getTempF(insideThermometer);
...
//Display Update
void DisplayWeather() {
	DisplayStatusSection(580, 23, wifi_signal);    // Wi-Fi signal strength and Battery voltage
	...
	DisplayMainWeatherSection(290, 110);           // Centre section of display for Location, temperature, Weather report, current Wx Symbol
...
void DisplayGeneralInfoSection() {
	setFont(OpenSans12B);
	drawString(5, 2, City + "             " + Date_str + "    " + Time_str + "       " + GetRoopTemp()+ "°F", LEFT);
}
void DisplayDisplayWindSection(int x, int y, float angle, float windspeed, int Cradius) {
	...
	setFont(OpenSans24B);
	drawString(x + 3, y - 21, String(windspeed, 1), CENTER);
	setFont(OpenSans12B);
	drawString(x, y + 15, (Units == "metric" ? "m/s" : "mph"), CENTER);
}
void DisplayAstronomySection(int x, int y) {
	setFont(OpenSans12B);
	drawString(x, y + 30, ConvertUnixTime(WxConditions[0].Sunrise).substring(0, 5) + " " + TXT_SUNRISE, LEFT);
	drawString(x, y + 55, ConvertUnixTime(WxConditions[0].Sunset).substring(0, 5) + " " + TXT_SUNSET, LEFT);
	time_t now = time(NULL);
	struct tm * now_utc  = gmtime(&now);
	const int day_utc    = now_utc->tm_mday;
	const int month_utc  = now_utc->tm_mon + 1;
	const int year_utc   = now_utc->tm_year + 1900;
	drawString(x, y + 80, MoonPhase(day_utc, month_utc, year_utc, Hemisphere), LEFT);
	DrawMoon(x + 170, y - 7, day_utc, month_utc, year_utc, Hemisphere);
}
void DisplayMainWeatherSection(int x, int y) {
	setFont(OpenSans10B);
	DisplayTemperatureSection(x, y - 40);
	DisplayForecastTextSection(x, y + 25);
	DisplayPressureSection(x, y + 90, WxConditions[0].Pressure, WxConditions[0].Trend);
}
void DisplayForecastSection(int x, int y) {
	...
	do { // Pre-load temporary arrays with with data - because C parses by reference and remember that[1] has already been converted to I units
		if (Units == "imperial") pressure_readings[r] = WxForecast[r].Pressure * 0.02953;   else pressure_readings[r] = WxForecast[r].Pressure;
		if (Units == "imperial") rain_readings[r]     = WxForecast[r].Rainfall * 0.0393701; else rain_readings[r]     = WxForecast[r].Rainfall;
		if (Units == "imperial") snow_readings[r]     = WxForecast[r].Snowfall * 0.0393701; else snow_readings[r]     = WxForecast[r].Snowfall;
		temperature_readings[r] = WxForecast[r].High;
	...
	DrawGraph(gx + 0 * gap, gy, gwidth, gheight, 900, 1050, Units == "metric" ? TXT_PRESSURE_HPA : TXT_PRESSURE_IN, pressure_readings, max_readings, autoscale_on, barchart_off);
	DrawGraph(gx + 1 * gap, gy, gwidth, gheight, 10, 30,    Units == "metric" ? TXT_TEMPERATURE_C : TXT_TEMPERATURE_F, temperature_readings, max_readings, autoscale_on, barchart_off);
	...
	DrawGraph(gx + 3 * gap + 5, gy, gwidth, gheight, 0, 30, Units == "metric" ? TXT_RAINFALL_MM : TXT_RAINFALL_IN, rain_readings, max_readings, autoscale_on, barchart_on);
	else
	DrawGraph(gx + 3 * gap + 5, gy, gwidth, gheight, 0, 30, Units == "metric" ? TXT_SNOWFALL_MM : TXT_SNOWFALL_IN, snow_readings, max_readings, autoscale_on, barchart_on);
}
...
void DisplayTemperatureSection(int x, int y) {
	setFont(OpenSans24B);
	drawString(x, y, String(WxConditions[0].Temperature, 1) + "°F    " + String(WxConditions[0].Humidity, 0) + "%", LEFT);
	setFont(OpenSans12B);
	drawString(x, y + 40, String(WxConditions[0].High, 0) + "°F | " + String(WxConditions[0].Low, 0) + "°F", LEFT); // Show forecast high and Low
}
...
if (WxForecast[0].Rainfall > 0) Wx_Description += " (" + String(WxForecast[0].Rainfall, 1) + String((Units == "metric" ? "mm" : "in")) + ")";
...
drawString(x, y + 8, TitleCase(Line1), LEFT);
if (Line1 != Line2) drawString(x, y + 33, Line2, LEFT);
}
void DisplayPressureSection(int x, int y, float pressure, String slope) {
	...
	if (WxConditions[0].Visibility > 0) {
		Visibility(x + 145, y, String(WxConditions[0].Visibility/1609.0) + "Mi");
...
void DisplayForecastWeather(int x, int y, int index) {
	...
	setFont(OpenSans10B);
	drawString(x + fwidth / 2, y + 30, String(GetUnixDay(WxForecast[index].Dt + WxConditions[0].Timezone)), CENTER);
...
void addsun(int x, int y, int scale, bool IconSize) {
	int linesize = 5;
	fillRect(x - scale * 2, y - (linesize/2), scale * 4, linesize, Black);
	fillRect(x- (linesize/2), y - scale * 2, linesize, scale * 4, Black);
...
void DrawGraph(int x_pos, int y_pos, int gwidth, int gheight, float Y1Min, float Y1Max, String title, float DataArray[], int readings, boolean auto_scale, boolean barchart_mode) {
	...
	drawFastVLine(x_pos + gwidth/7 * 0 + gwidth/7, y_pos, gheight, LightGrey);
    drawFastVLine(x_pos + gwidth/7 * 1 + gwidth/7, y_pos, gheight, LightGrey);
    drawFastVLine(x_pos + gwidth/7 * 2 + gwidth/7, y_pos, gheight, LightGrey);
    drawFastVLine(x_pos + gwidth/7 * 3 + gwidth/7, y_pos, gheight, LightGrey);
    drawFastVLine(x_pos + gwidth/7 * 4 + gwidth/7, y_pos, gheight, LightGrey);
    drawFastVLine(x_pos + gwidth/7 * 5 + gwidth/7, y_pos, gheight, LightGrey);
    drawFastVLine(x_pos + gwidth/7 * 6 + gwidth/7, y_pos, gheight, LightGrey);
    drawFastVLine(x_pos + gwidth/7 * 7 + gwidth/7, y_pos, gheight, LightGrey);
    if(barchart_mode)
    {
        drawString(x_pos + ((gwidth/7) * 0), y_pos + gheight + 10, "1d", LEFT);
        drawString(x_pos + ((gwidth/7) * 2), y_pos + gheight + 10, "3d", LEFT);
        drawString(x_pos + ((gwidth/7) * 4), y_pos + gheight + 10, "5d", LEFT);
        drawString(x_pos + ((gwidth/7) * 6), y_pos + gheight + 10, "7d", LEFT);
    }
    else
    {
        drawString(x_pos-13 + ((gwidth/7) * 1), y_pos + gheight + 10, "1d", LEFT);
        drawString(x_pos-13 + ((gwidth/7) * 3), y_pos + gheight + 10, "3d", LEFT);
        drawString(x_pos-13 + ((gwidth/7) * 5), y_pos + gheight + 10, "5d", LEFT);
        drawString(x_pos-13 + ((gwidth/7) * 7), y_pos + gheight + 10, "7d", LEFT);
    }

}
void DrawPressureAndTrend(int x, int y, float pressure, String slope) {
	drawString(x + 25, y - 10, String(pressure, (Units == "metric" ? 0 : 1)) + (Units == "metric" ? "hPa" : "in"), LEFT);
	if      (slope == "+") {
...
void DrawMoon(int x, int y, int dd, int mm, int yy, String hemisphere) {
	Rect_t area = {.x = x+(moon_width*.5), .y = y+(moon_height*.5), .width  = moon_width, .height =  moon_height};
...
void DrawBattery(int x, int y) {
	...
	float voltage = esp_adc_cal_raw_to_voltage(adc_reading, &adc_chars) * 2 /1000.0;
	Serial.println(String(voltage));
	uint8_t percentage = -66.787 * pow(voltage,4) + 845.48 * pow(voltage,3) - 3870.5 * pow(voltage,2) + 7583.3 * voltage - 5331.4;
	if (voltage >= 4.185) percentage = 100;
	if (voltage <= 3.00) percentage = 0;  // orig 3.5
	drawRect(x + 25, y - 14, 40, 15, Black);
	fillRect(x + 65, y - 10, 4, 7, Black);
	fillRect(x + 27, y - 12, 36 * percentage / 100.0, 11, Black);
	drawString(x + 75, y - 14, String(percentage) + "%  " + String(voltage, 2) + "v", LEFT);
}
